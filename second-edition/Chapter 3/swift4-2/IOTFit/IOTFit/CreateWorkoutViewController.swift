...
class CreateWorkoutViewController: UIViewController {
    ...
    @objc func updateWorkoutData() {
        ...
        workoutDistanceLabel?.text = String(format: "%.2fm | %d steps | %d floors", arguments: [workoutDistance, workoutSteps, floorsAscended])
    ...
    func startPedometerUpdates() {
        ...
        guard let lastSavedTime = lastSavedTime else {
            return
        }
        
        pedometer = CMPedometer()
        pedometer?.startUpdates(from: lastSavedTime, withHandler: { [weak self] (pedometerData : CMPedometerData?, error: Error?) in
