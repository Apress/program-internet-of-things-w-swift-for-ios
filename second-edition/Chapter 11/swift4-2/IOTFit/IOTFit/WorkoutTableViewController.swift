...
class WorkoutTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        notificationCenter.addObserver(self, selector: #selector(showSecurityView), name: UIApplication.willResignActiveNotification, object: nil)
    }
    ...
    @objc func showSecurityView() {
        if let securityView = self.securityView, securityView.isHidden == true {
            ...
            view.bringSubviewToFront(securityView)
        }
    ...
    func didFinishWithAuthenticationType(_ type: AuthenticationType) {
        UIView.animate(withDuration: 0.3, animations: { [weak self] in
            DispatchQueue.main.async {
                ...
                self?.view.sendSubviewToBack(securityView)
                self?.tableView.reloadData()
            }
        })
    }
